...
public class XmlDiff {
	...
	public static void Main(string[] args) {
		...
		test.AddInterface(typeof(string), new StringNodeInterface());
...
public class StringNodeInterface : NodeInterface, XmlOutputNodeInterface {
...
public class XmlNodeInterface : NodeInterface, XmlOutputNodeInterface {
	...
	public override IList GetChildren(object item) {
		...
		if (!(item is XmlDocument || item is XmlElement)
			|| (item is XmlElement && node.Attributes.Count == 0 && node.ChildNodes.Count == 1 && node.FirstChild is XmlText)) {
			string[] lines = ((XmlNode)item).InnerText.Split('\n', '\r');
	...
	public override float Compare(object left, object right, StructuredDiff comparer) {
		...
		if (cleft == null || cright == null) {
			ret = (l.InnerText == r.InnerText) ? 0 : 1;
		} else {		
